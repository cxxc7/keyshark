    ...
    <header className="w-full h-12 sm:h-14 flex items-center justify-between px-4 sm:px-8 bg-white/80 dark:bg-zinc-900/80 border-b border-zinc-200 dark:border-zinc-800 shadow-sm fixed top-0 left-0 z-30 backdrop-blur">
      <div className="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div className="flex-1 flex items-center min-w-0 mx-4 overflow-hidden">
          <div className="w-full overflow-hidden whitespace-nowrap">
            <div
              className="animate-marquee inline-block min-w-full text-xs sm:text-sm font-medium opacity-80"
              style={{ animation: 'marquee 30s linear infinite', color: 'var(--accent-color, #2563eb)' }}
            >
              <span style={{ display: 'inline-block', minWidth: '100%' }}>
                ...
                {features.map((f) => (
                  <span key={f + '-repeat'} className="mx-6"></span>
                ))}
              </span>
            </div>
          </div>
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex items-center">
          ...
          >
            <span
              className="inline-block w-4 h-4 rounded-full border-2 border-zinc-300 dark:border-zinc-700"
            ...
            <svg className="w-3 h-3 ml-1 text-zinc-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 20 20">
              <path d="M6 8l4 4 4-4" />
            </svg>
          </button>
          {showPalette && (
            <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-lg p-4 flex flex-col gap-2 z-50 w-full min-w-[320px] max-w-[420px]">
              ...
              <div className="flex flex-wrap justify-center gap-2 text-xs text-zinc-700 dark:text-zinc-300">
                {COLORS.map((c) => (
                  ...
                  >
                    <span
                      className={`inline-block w-7 h-7 rounded-full border-2 border-zinc-300 dark:border-zinc-700 ${c.bg}`}
                    />
                    <span>{c.name}</span>
                  </button>
                ))}
        ...
        <button
          className="p-2 rounded hover:bg-blue-100 dark:hover:bg-zinc-800 flex items-center gap-2 transition ml-2"
          aria-label="Account"
        ...
        >
          {hydrated && profilePic ? (
            <Image
              ...
              width={32}
              height={32}
              className="rounded-full object-cover border-2 border-blue-300 dark:border-zinc-700 shadow"
            />
          ) : (

...
export default function SignupPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        let idx = accounts.findIndex((acc: any) => acc.username === username);
        if (idx === -1) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-200 via-white to-blue-100 dark:from-zinc-900 dark:via-zinc-950 dark:to-blue-900 p-2 xs:p-4">
      <form onSubmit={handleSignup} className="bg-white/90 dark:bg-zinc-900/90 p-6 xs:p-10 rounded-3xl shadow-2xl w-full max-w-xs xs:max-w-md flex flex-col gap-5 xs:gap-7 border border-zinc-200 dark:border-zinc-800 backdrop-blur">
        <h1 className="text-2xl xs:text-4xl font-extrabold text-center mb-2 text-blue-700 dark:text-blue-200 tracking-tight drop-shadow">Create Account</h1>
        <p className="text-center text-zinc-500 dark:text-zinc-400 mb-2 text-sm xs:text-base">Sign up to get started</p>
        <div className="relative">
          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 pointer-events-none">
            {/* User icon */}
            <svg width="20" height="20" className="xs:w-[22px] xs:h-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 7.5v.008M12 12a4.5 4.5 0 1 0 0-9 4.5 4.5 0 0 0 0 9Zm0 0c-3.038 0-5.5 2.462-5.5 5.5v.25A2.25 2.25 0 0 0 8.75 20h6.5a2.25 2.25 0 0 0 2.25-2.25v-.25c0-3.038-2.462-5.5-5.5-5.5Z"/></svg>
          </span>
          <input
            type="text"
        ...
        <div className="relative">
          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 pointer-events-none">
            {/* Lock icon */}
            <svg width="20" height="20" className="xs:w-[22px] xs:h-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V7a4.5 4.5 0 0 0-9 0v3.5m-1.5 0A1.5 1.5 0 0 0 4.5 12v6A1.5 1.5 0 0 0 6 19.5h12a1.5 1.5 0 0 0 1.5-1.5v-6a1.5 1.5 0 0 0-1.5-1.5m-13.5 0h15"/></svg>
          </span>
          <input
          ...
          >
            {showPassword ? (
              ...
              // Eye icon
              <svg width="20" height="20" className="xs:w-[22px] xs:h-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 5c-7 0-9 7-9 7s2 7 9 7 9-7 9-7-2-7-9-7Zm0 0a3 3 0 1 1 0 6 3 3 0 0 1 0-6Z"/></svg>
            )}
          </button>
        </div>
        ...
        <div className="text-xs xs:text-sm text-center text-zinc-500 mt-2">
          Already have an account? <a href="/login" className="text-blue-600 dark:text-blue-400 underline font-semibold">Login</a>
        </div>
      </form>
    </div>
  );

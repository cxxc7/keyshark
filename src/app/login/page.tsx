  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        const idx = accounts.findIndex((acc) => acc.username === email);
        if (idx === -1) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-200 via-white to-blue-100 dark:from-zinc-900 dark:via-zinc-950 dark:to-blue-900 px-2 xs:px-4">
      <form
        ...
        className="bg-white/90 dark:bg-zinc-900/90 p-6 xs:p-8 sm:p-10 rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-xs xs:max-w-sm sm:max-w-md flex flex-col gap-5 xs:gap-6 sm:gap-7 border border-zinc-200 dark:border-zinc-800 backdrop-blur"
      >
        <h1 className="text-2xl xs:text-3xl sm:text-4xl font-extrabold text-center mb-1 xs:mb-2 text-blue-700 dark:text-blue-200 tracking-tight drop-shadow">
          Welcome Back
        </h1>
        <p className="text-center text-zinc-500 dark:text-zinc-400 mb-1 xs:mb-2 text-sm xs:text-base">
        ...
        {error && (
          <div className="text-red-500 text-xs xs:text-sm text-center font-semibold bg-red-50 dark:bg-zinc-800 rounded-lg py-2 px-3">
            {error}
          </div>
        )}
        ...
        <div className="text-xs xs:text-sm text-center text-zinc-500 mt-1 xs:mt-2">
          ...
          <a href="/signup" className="text-blue-600 dark:text-blue-400 underline font-semibold">
            Sign Up
          </a>
        </div>
      </form>
